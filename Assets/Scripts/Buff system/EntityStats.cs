...
public abstract class EntityStats : MonoBehaviour
{
    ...
    //if applying a biff via BuffInfo, we will also check the probability first
    ...
    public virtual bool ApplyBuff(BuffData data, int variant = 0, float durationMultiplier = 1f)
    {
        ...
        switch (s.stackType)
        {
            //if the buff stacks fully, we can have multiple copies of the buff
            case BuffData.StackType.stacksFully:
                activeBuffs.Add(new Buff(data, this, variant, durationMultiplier));
                RecalculateStats();
                return true;

            //if it only refreshes the duration, we will find the buff that is already there
            //and reset the remaining duration (if the buff is not there yet). Otherwise, we just add the buff
    ...
    public virtual bool RemoveBuff(BuffData data, int variant = -1)
    {
        ...
        //we need to remove the buffs outsife of the loop, otherwise this
    ...
    protected virtual void UpdateColor()
    {
        ...
        //set all our sprites' color to the computed target color
    ...
    protected virtual void Update()
    {
        ...
        //we remove the buffs outside the foreach loop, as it will affect the iteration if we remove items from th
        //list while a loop is still running
